...
def build_rx_features_with_other(prescriptions: pd.DataFrame) -> pd.DataFrame:
    ...
    # Merge in other prescription features
    out = out.merge(other_agg, on="subject_id", how="left")
    for col in ["other_rx_count", "other_hadms", "distinct_others", "other_exposure_days"]:
        if col in out.columns:
            out[col] = out[col].fillna(0)
    out["any_other_rx_flag"] = (out["other_rx_count"].fillna(0) > 0).astype(int)
    return out
...
def main():
    ...
    rx = build_rx_features_with_other(prescriptions)
    ...
    for col in ["n_hospital_admits", "avg_los_days", "total_los_days",
                ...
                "other_rx_count", "other_hadms", "distinct_others", "other_exposure_days", "any_other_rx_flag"]:
        if col in df.columns:
            df[col] = df[col].fillna(0)
    df["y_oud"] = df["y_oud"].fillna(0).astype(int)
    df["will_get_opioid_rx"] = df["any_opioid_flag"].fillna(0).astype(int)
    out_csv = "mimiciv_demo_oud_features_with_other_rx.csv"
