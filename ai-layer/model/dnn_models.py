...
def validate(model, val_loader, criterion, device):
    ...
    with torch.no_grad():
        for features, labels in val_loader:
            ...
            outputs_clamped = torch.clamp(
                outputs, min=-10, max=10
    ...
    try:
        ...
        if np.any(np.isnan(all_probs_array)) or np.any(np.isinf(all_probs_array)):
            ...
            all_probs_array = np.nan_to_num(
                all_probs_array, nan=0.5, posinf=0.999, neginf=0.001
            )
...
def evaluate_test_set(model, test_loader, device):
    ...
    metrics = {
        ...
        "auc": (
            roc_auc_score(all_labels, all_probs) if len(set(all_labels)) > 1 else 0.5
        ),
    }
...
def train_single_model(
    train_df, val_df, test_df, target, feature_cols, model_name, output_dir, device
):
    ...
    # Standardize features. To make all the features have similar ranges. Scales the features to have mean=0 and standard deviation=1
    ...
    model = DeepNet(
        input_dim=input_dim, hidden_dims=[128, 64, 32, 16], dropout_rate=0.3
    )
    model = model.to(device)

    # Loss and optimizer
    raw_pos_weight = weight_for_1 / weight_for_0
    pos_weight = torch.tensor([min(raw_pos_weight, 10.0)]).to(device)
    ...
    metrics, cm, preds, probs, labels = evaluate_test_set(
    ...
    pred_df = pd.DataFrame(
        {target: labels, "prediction_label": preds, "prediction_score": probs}
    )
